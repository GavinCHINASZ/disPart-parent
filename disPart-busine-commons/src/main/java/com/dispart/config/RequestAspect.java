...
public class RequestAspect {
    ...
    @Pointcut("execution(* com.dispart.controller.*.*(..)) " +
            "&& !execution(* com.dispart.controller.AuthController.*(..))" +
            "&& !execution(* com.dispart.controller.InternetBankController.*(..))" +
            "&& !execution(* com.dispart.controller.PayJrnController.*(..))" +
            "&& !execution(* com.dispart.controller.HsbController.customInfoCallback(..))"
    )
    public  void handle(){
        //全局处理
    }
    ...
    public Result singBefore(JoinPoint joinPoint){
        log.info("业务公共服务-客户端请求地址URL，{}",req.getRequestURL());
        Object object=joinPoint.getArgs();
        if(object!=null){
            log.info("业务公共服务-客户端请求参数，{}" + JSONObject.toJSONString(joinPoint.getArgs()));
            ...
            log.debug("业务公共服务-记录客户端请求日志记录开始......");
            //需增加表
        ...
        //处理请求验签
        log.debug("业务公共服务-处理客户端请求验签开始.......");
        String node_ip=req.getHeader("node-ip");
        log.debug("业务公共服务-客户端请求签名参数node_ip,{}",node_ip);
        String node_name=req.getHeader("node-name");
        log.debug("业务公共服务-客户端请求签名参数node_name,{}",node_name);
        String request_time=req.getHeader("request-time");
        log.debug("业务公共服务-客户端请求签名参数request_time,{}",request_time);
        String sign=req.getHeader("sign");
        log.debug("业务公共服务-客户端签名值sign,{}",sign);
        try {
            log.debug("业务公共服务-业务端验签秘钥值sign_secret,{}",sign_secret);
            String newSign= SHAUtils.getSHA256Str(sign_secret,"node-ip="+node_ip+"&node-name="+node_name+"&request-time="+request_time);
            log.debug("业务公共服务-业务端验签值sign,{}",newSign);
        if(newSign.equals(sign)){
            log.info("业务公共服务-客户端请求验签成功");
        }else {
            log.error("业务公共服务-客户端请求验签失败");
        }
        } catch (Exception e) {
            log.error("业务公共服务-客户端请求出现异常", e);
        }
        log.debug("业务公共服务-处理客户端请求验签结束.......");
