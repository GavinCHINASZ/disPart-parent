...
//    @Pointcut("execution(* com.dispart.controller.*.*(..))")
    ...
    @Pointcut("execution(* com.dispart.controller.*.*(..)) " +
            "&& !execution(* com.dispart.controller.OrderController.orderResultCallBack(..))" +
            "&& !execution(* com.dispart.controller.OrderController.verifyAccountCallBack(..))" +
            "&& !execution(* com.dispart.controller.OtherOrderController.resultToHSBOnlineWithdraw(..))"
    )
    public void handle(){
        //全局处理
    }
    ...
    public Result singBefore(JoinPoint joinPoint){
        ...
        if(!url.contains("/test/down")
            ...
            //处理请求验签
            log.info("业务公共服务-处理客户端请求验签开始.......");
            String node_ip=req.getHeader("node-ip");
            log.info("业务公共服务-客户端请求签名参数node_ip,{}",node_ip);
            String node_name=req.getHeader("node-name");
            log.info("业务公共服务-客户端请求签名参数node_name,{}",node_name);
            String request_time=req.getHeader("request-time");
            log.info("业务公共服务-客户端请求签名参数request_time,{}",request_time);
            String sign=req.getHeader("sign");
            log.info("业务公共服务-客户端签名值sign,{}",sign);
            try {
                log.info("业务公共服务-业务端验签秘钥值sign_secret,{}",sign_secret);
                String newSign= SHAUtils.getSHA256Str(sign_secret,"node-ip="+node_ip+"&node-name="+node_name+"&request-time="+request_time);
                log.info("业务公共服务-业务端验签值sign,{}",newSign);
                if(sign.equals(newSign)){
                    log.info("业务公共服务-客户端请求验签成功");
                }else {
                    log.info("业务公共服务-客户端请求验签失败");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            log.info("业务公共服务-处理客户端请求验签结束.......");
        }
        return null;
    }
}